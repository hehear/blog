---
title: redis系列-springboot集成redis
date: 2019-08-23 23:14:27
categories:
  - redis
tags:
  - redis
...
### springboot集成redis
maven pom.xml添加spring-data-redis依赖
```java
        <dependency>
...
```
### 配置redis
新增application-redis.properties配置文件
```xml
# Redis数据库索引（默认为0）
...
```
在application.properties引入redis配置
```xml
#引入其他配置文件
spring.profiles.active= datasource,mybatis,tomcat,redis
```
### reidsTemplate配置
新建redisConfig配置类，对String、hash的key、value进行序列化
```java
@Configuration
